...
/**
 * Overrides the binding lookup. Used by tests and by non-Cloudflare hosts; on
 * Cloudflare the binding is discovered automatically below.
 */
...
/**
 * Resolves the `API` service binding when running on Cloudflare.
 *
 * `getCloudflareContext` is imported lazily so that `next build`, `next dev` on
 * Node, and any non-Cloudflare host never even load the adapter. A missing
 * binding is not an error — the HTTP transport is the documented fallback.
 */
async function apiBinding(): Promise<ApiBinding | null> {
  const override = (globalThis as unknown as BindingHolder).__PD_API_BINDING;
  if (override) return override;

  try {
    const { getCloudflareContext } = await import('@opennextjs/cloudflare');
    // Double cast: `CloudflareEnv` is generated by `wrangler types` and is not
    // present on a fresh checkout, so this must not depend on its shape.
    const env = getCloudflareContext().env as unknown as Record<string, unknown> | undefined;
    const binding = env?.API;
    if (binding && typeof (binding as ApiBinding).fetch === 'function') {
      return binding as ApiBinding;
    }
  } catch {
    // Not running on Cloudflare, or the context is not available in this scope.
  }
  return null;
}
...
export async function apiRequest<T>(path: string, options: ApiRequestOptions = {}): Promise<T> {
  ...
  const binding = await apiBinding();
