import { escapeHtml } from '@pd/shared';
 ...
 * Transactional email.
 *
 * The `console` provider is the default so the platform runs end to end without
 * a paid service — links land in the Worker log. Resend is the supported hosted
 * provider; it is called over plain `fetch`, so no SDK is bundled into the
 * Worker.
 */
...
export function mailer(): EmailAdapter {
  ...
  if (provider === 'resend' && apiKey) {
    return new ResendAdapter(apiKey, env.EMAIL_FROM ?? 'promptduniya <no-reply@promptduniya.in>');
  }
  ...
  return mailer().send({
    ...
    html: layout(
      ...
      `<h1 style="margin:0 0 12px;font-size:22px">You're premium 🎉</h1>
       ...
       ${button(`${config().webOrigin}/dashboard`, 'Open your dashboard')}`,
    ),
