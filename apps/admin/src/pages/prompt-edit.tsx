  ...
  async function generateCover() {
    ...
    setCoverNote(
      `Generated by ${result.engine}${result.usedReference ? ' using the house model' : ''}. Save the prompt to keep it.` +
        // Without this the note named the fallback engine but not the cause, so a
        // cover drawn by the weaker model read as a bad prompt rather than as an
        // exhausted quota or a wrong model id.
        (result.fallbackReason
          ? `\n\nThe preferred model did not run, so this came from the fallback — expect lower quality. ${result.fallbackReason}`
          : ''),
    );
  }
            ...
            <div className="space-y-4">
              ...
              {id ? (
                <div className="space-y-2">
                  ...
                  {coverNote && (
                    // `whitespace-pre-line` because the note carries a second
                    // paragraph when a fallback ran, and HTML would collapse it
                    // onto the end of the first sentence.
                    <p className="whitespace-pre-line rounded-lg bg-[var(--surface-sunken)] px-2.5 py-2 text-xs text-[var(--text-body)]">
                      {coverNote}
                    </p>
                  )}
                </div>
